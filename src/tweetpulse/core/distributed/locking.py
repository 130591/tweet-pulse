...
class RedisLock:
  ...
  def __init__(self, redis_client: Redis, lock_key: str,
  ...
  async def extend(self, additional_seconds: int = None) -> bool:
    ...
    if result:
      logger.debug(
          f"Distributed lock extended: {self.lock_key} (+{additional_seconds}s)")
      return True
    else:
...
class DistributedLockManager:
  ...
  async def acquire_lock(self, lock_name: str,
                         timeout_seconds: int = 30) -> Optional[RedisLock]:
    """
